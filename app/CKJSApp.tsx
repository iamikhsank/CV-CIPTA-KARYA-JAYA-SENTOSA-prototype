...
function DashboardView({ go, newTransaction, transfer, cashAccounts }: { go: (view: View) => void; newTransaction: () => void; transfer: () => void; cashAccounts: typeof cashSeed }) {
  ...
  return <>
    ...
    <section className="kpi-grid" aria-label="Ringkasan keuangan">{kpis.map((item) => <article className={`kpi-card ${item.tone}`} key={item.label}><div className="kpi-body"><div className="kpi-top"><span>{item.label}</span><b>{item.delta}</b></div><strong>{item.value}</strong><p>{item.meta}</p></div><button className="kpi-footer" onClick={() => go(item.label === "Total Cash" ? "Cash Accounts" : item.label === "Expense" ? "Transactions" : "Reports")} type="button">View more <span>→</span></button></article>)}</section>
