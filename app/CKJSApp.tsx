...
function DashboardView({ go, newTransaction, transfer, cashAccounts }: { go: (view: View) => void; newTransaction: () => void; transfer: () => void; cashAccounts: typeof cashSeed }) {
  const kpis = [
    { label: "Total Cash", value: formatIDR(cashAccounts.reduce((sum, item) => sum + item.balance, 0)), delta: "+8,4%", tone: "green", meta: "dari bulan lalu", icon: Wallet02Icon },
    { label: "Revenue", value: "Rp 1.284.000.000", delta: "+12,5%", tone: "blue", meta: "periode ini", icon: MoneyReceive01Icon },
    { label: "Expense", value: "Rp 846.750.000", delta: "+4,2%", tone: "amber", meta: "periode ini", icon: MoneySend01Icon },
    { label: "Net Profit", value: "Rp 437.250.000", delta: "34,1%", tone: "purple", meta: "profit margin", icon: Chart01Icon },
  ];
  return <>
    ...
    <section className="kpi-grid" aria-label="Ringkasan keuangan">{kpis.map((item) => <article className={`kpi-card ${item.tone}`} key={item.label}><div className="kpi-body"><div className="kpi-top"><span>{item.label}</span><i><HugeiconsIcon icon={item.icon} size={17} strokeWidth={1.7} /></i></div><div className="kpi-value"><strong>{item.value}</strong><b>{item.delta}</b></div><p>{item.meta}</p></div><button className="kpi-footer" onClick={() => go(item.label === "Total Cash" ? "Cash Accounts" : item.label === "Expense" ? "Transactions" : "Reports")} type="button">View more <span>→</span></button></article>)}</section>
